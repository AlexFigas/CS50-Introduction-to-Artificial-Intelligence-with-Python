...
def load_data(filename):
    ...
    # Dataframe evidence
    evidence = file.drop(columns=['Revenue'])

    # Replace Month String with Integer
    evidence = evidence.replace(months)

    # Replace Boolean with 0 or 1 Integer
    evidence['VisitorType'] = evidence['VisitorType'].apply(
        lambda l: 1 if l == 'Returning_Visitor' else 0
    )
    evidence['Weekend'] = evidence['Weekend'].apply(
        lambda l: 1 if l == 'True' else 0
    )
    labels = labels.apply(lambda l: 1 if l is True else 0)
