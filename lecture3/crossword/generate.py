...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        for var in self.domains:
            for word in self.domains[var].copy():  # Copy to avoid error
                if len(word) != var.length:
                    # If doesn't fit, remove from domain
    ...
    def revise(self, x, y):
        ...
        if overlap is None:
            # If there's no overlap, there won't be any conflicts so no revision, return False
            return False

        for first_word in self.domains[x].copy():  # Copy to avoid error
            delete = True  # Word will be deleted unless there's a valid second word

            for second_word in self.domains[y]:
                if first_word[overlap[0]] == second_word[overlap[1]]:
                    delete = False
                    break

            if delete:
                # If no valid second word, delete first word
    ...
    def ac3(self, arcs=None):
        ...
        while arcs:
            ...
            if self.revise(x, y):
                ...
                # If after revision, domain is empty, return False (no solution)
...
#############################################################################################################
    ...
    def consistent(self, assignment):
        ...
        for variable in assignment:
            ...
            # Check if word fits in crossword
